...
void NMEA_ParseGGA(char command[], NmeaInfo *info);
...
void NMEA_ParseCommand(char command[], size_t length, NmeaInfo *info) {
    ...
    // GPS定位信息
    if (strstr(command, "GGA") != NULL) {
        NMEA_ParseGGA(command, info);
        return;
    }
    // 推荐定位信息
...
void NMEA_ParseGGA(char command[], NmeaInfo *info) {
    ...
    while (NMEA_ParseNextValue(&pointer, value, &length, &index) > 0) {
        ...
        switch (index) {
            case 9:
                info->altitude = strtof(value, NULL);
                break;
            case 11:
                info->ellipsoid = strtof(value, NULL);
                break;
            default:
                break;
        }
